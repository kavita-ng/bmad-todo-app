...
async function errorHandlerPlugin(app: FastifyInstance) {
  app.setErrorHandler((error, _request, reply) => {
    ...
    const statusCode = Number.isInteger(rawStatusCode) && rawStatusCode >= 100 && rawStatusCode < 600 ? rawStatusCode : 500
    
    return reply.status(statusCode).send({
      error: {
        code: statusCode === 404 ? 'NOT_FOUND' : 'INTERNAL_SERVER_ERROR',
        message: statusCode < 500 ? (error.message ?? 'An unexpected error occurred') : 'An unexpected error occurred',
      },
    })
  })
